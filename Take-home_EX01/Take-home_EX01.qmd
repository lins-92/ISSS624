...
We can also plot it as proportions of total water points. First, we need to generate new variables for proportions. There are some administrative areas without any waterpoints which will result is na values for the proportions. We **do not replace these na values** with 0 because it will affect the subsequent analysis. There are 2 problems with replacing them with 0:

-   0 values represent a low proportion of functional/non-functional waterpoints. This is inaccurate since there were no waterpoints at all. Using 0 may skew the analysis on spatial distribution and clustering.

-   These areas will have low proportions for both functional and non-functional waterpoints. The relationship between these should be approximately inverse.
...
In this exercise, I will fix the number of neighbours at 8 \[[1](https://pro.arcgis.com/en/pro-app/latest/tool-reference/spatial-statistics/generate-spatial-weights-matrix.htm)\].
...
The local Moran's score alone is not enough to show spatial clustering because it does not tell us whether the value of the variable being tested (proportion of functional/non-functional waterpoints) and whether the test result was significant. As such, we assign each observation to a quadrant depending on the value of the variable on the y-axis (centred around the mean) and Moran's I on the x-axis. The following table explains the the quadrants:

![](images/scatterquad.png){width="1900"}

The following code 2 new variables each for proportion of functional and non-functional waterpoints. One for the centred proportion (around the mean) and the quadrant that the observation belong to. Note that we must include the `na.rm=TRUE` argument when computing mean because our data has na values.
...
The LISA cluster maps clearly show that there is clustering in the proportion of functional/non-functional waterpoints. There is a large cluster of regions with high proportion of functional waterpoints in the central north of Nigeria. There is also a cluster of low proportion of functional waterpints in the southeast. This area is an area of concern for maintenance efforts. There are not many outliers.

There is a large cluster of low proportion of non-functional waterpoints in the central north of Nigeria that overlaps with the cluster of high proportion functional waterpoint cluster. This is logical as proportions are inversed. If the proportion of functional waterpoints is high, the proportion of non-functional waterpoints should be low.

However, we see a different story in the south where there is a cluster of high proportion of non-functional waterpoint areas and no corresponding cluster of low proportion of functional waterpoint areas. This could be due to a difference in the centring of the proportions. From the code chunk below, we see that the mean proportion of non-functioning waterpoints is much lower than that of functional waterpoints (because there are unknowns as well). The threshold to be classified as a high non-functional cluster is lower than the threshold to be classified as a low functional cluster.
